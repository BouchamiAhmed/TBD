...
// Time-varying hash for animation
float hashTime(vec2 p, float t) {
    return fract(sin(dot(p + vec2(t * 0.1), vec2(127.1, 311.7))) * 43758.5453123);
}

// Color palette for each item
vec3 getItemColor(int index) {
    vec3 colors[10];
    colors[0] = vec3(0.18, 0.56, 0.37); // K3s - Green
    colors[1] = vec3(0.38, 0.86, 0.64); // LDAP - Bright Green
    colors[2] = vec3(0.38, 0.70, 0.86); // Traefik - Blue
    colors[3] = vec3(0.49, 0.53, 0.88); // Security - Indigo
    colors[4] = vec3(0.65, 0.33, 0.92); // Multi-Tenant - Purple
    colors[5] = vec3(0.97, 0.45, 0.06); // gRPC - Orange
    colors[6] = vec3(0.06, 0.72, 0.51); // Platform - Emerald
    colors[7] = vec3(0.06, 0.65, 0.91); // Monitor - Cyan
    colors[8] = vec3(0.94, 0.27, 0.27); // Longhorn - Red
    colors[9] = vec3(0.96, 0.62, 0.04); // CI/CD - Amber
...
void main() {
    ...
    vec3 baseColor = getItemColor(itemIndex);

    // Create circular disc
    vec2 centerOffset = vUvs - 0.5;
    float dist = length(centerOffset);

    // Circular mask
    float circle = smoothstep(0.5, 0.48, dist);

    // Inner gradient from center (darker) to edge (brighter)
    float gradient = smoothstep(0.0, 0.5, dist);

    // Darker center, brighter edges
    vec3 darkColor = baseColor * 0.4;
    vec3 brightColor = baseColor * 0.85;
    vec3 finalColor = mix(darkColor, brightColor, gradient);

    // Solid bubble with slight transparency
    float finalAlpha = circle * 0.75 * vAlpha;
...
interface MenuItem {
  link: string;
  title: string;
  bubbleTitle?: string; // Short title for bubble (e.g., "K3s HA")
  description: string;
  color?: string;
  points?: string[];
}
...
class InfiniteGridMenu {
  ...
  private init(onInit?: InitCallback): void {
    ...
    this.discLocations = {
      ...
      uTime: gl.getUniformLocation(this.discProgram!, 'uTime')
    };
  ...
  private initTexture(): void {
    ...
    // Create a simple 1x1 white texture as placeholder (shader generates patterns)
  ...
  private render(): void {
    ...
    gl.uniform1f(this.discLocations.uTime, this._time / 1000.0); // Convert to seconds
...
const InfiniteMenu: FC<InfiniteMenuProps> = ({ items = [] }) => {
  ...
  // Animate points with power2 ease
  useEffect(() => {
    if (!pointsRef.current) return;

    const pointElements = pointsRef.current.querySelectorAll('.point-item');
  ...
  return (
    <div className="relative w-full h-full bg-transparent">
      ...
      <div className="absolute top-8 left-1/2 transform -translate-x-1/2 pointer-events-none">
        <div className="flex flex-wrap gap-4 justify-center max-w-4xl">
          {items.slice(0, 10).map((item, index) => (
            <div
              key={index}
              className="text-white font-semibold text-xs tracking-wide backdrop-blur-sm bg-black/20 px-3 py-1 rounded-full border"
              style={{
                fontFamily: 'Georgia, serif',
                fontStyle: 'italic',
                borderColor: item.color || '#ffffff',
                color: item.color || '#ffffff',
                textShadow: `0 0 8px ${item.color || '#ffffff'}60`
              }}
            >
              {item.bubbleTitle || item.title}
            </div>
          ))}
        </div>
      </div>
      ...
      {activeItem && (
        <>
          {/* Bubble Title (centered on bubble) */}
          <h3
            style={{
              color: activeItem.color || '#ffffff',
              textShadow: `0 0 30px ${activeItem.color || '#ffffff'}dd, 0 4px 12px rgba(0,0,0,1)`,
              fontFamily: 'Georgia, serif',
              fontStyle: 'italic',
          ...
          ${
            isMoving
              ? 'opacity-0 pointer-events-none duration-[150ms] scale-80'
              : 'opacity-100 pointer-events-auto duration-[600ms] scale-100'
          }
        `}
          >
            {activeItem.bubbleTitle || activeItem.title}
          </h3>
          ...
          <p
            style={{
              color: activeItem.color || '#ffffff',
              textShadow: `0 0 20px ${activeItem.color || '#ffffff'}80`
            }}
            className={`
          select-none
          absolute
          ...
          ${
            isMoving
              ? 'opacity-0 pointer-events-none duration-[150ms] translate-y-[200px]'
              : 'opacity-100 pointer-events-auto duration-[600ms] translate-y-[180px]'
          }
        `}
          >
          ...
          {activeItem.points && activeItem.points.length > 0 && (
            ...
            >
              {activeItem.points.map((point, index) => (
                <div
                  key={index}
                  className="point-item flex items-start gap-3 mb-3 opacity-0"
                  style={{
                    borderLeft: `3px solid ${activeItem.color || '#ffffff'}`,
                    paddingLeft: '12px'
                  }}
                >
                  <span
                    style={{
                      color: activeItem.color || '#ffffff',
                      textShadow: `0 0 10px ${activeItem.color || '#ffffff'}60`
                    }}
